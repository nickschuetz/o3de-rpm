...
| 4: OpenSSL 3 migration | **MOVING (2026-07), the wild card is finally in motion.** 3p [#387](https://github.com/o3de/3p-package-source/pull/387) "Bump OpenSSL 1.1.1 -> 3.6.3 across all platforms" **MERGED 2026-07-23** (RobotecAI's Jan Hanca): the 3rdParty OpenSSL package is now 3.6.3, retiring the EOL 1.1.1t at the package level. Next: an engine-side PR must bump the `OpenSSL-1.1.1t-rev1-linux` pin in BuiltInPackages to consume 3.6.3 (our dev-tip builds still statically link 1.1.1t as of 20260726, so nothing changes downstream until that lands). Our 3p [#376](https://github.com/o3de/3p-package-source/pull/376) (system-OpenSSL variant for the bundled Python 3.10) is still open/in-review; it is the real Fedora win (a 3.6.3 bundle is still a bundle; Fedora wants the system lib). Fires our commitment to rebase #376 onto merged #387 + re-validate, and to reword the `bundling_exception: OpenSSL` "EOL 1.1.1t" rationale (dep-map.yaml + BUNDLED_LIBRARIES.md). Separately measured 2026-07-22: AzFramework statically exports 1.1.1t unhidden, colliding with a Gem's OpenSSL 3.x via versioned-vs-unversioned symbol shadowing (posted on #376); deeper fix is hidden-visibility on AzNetworking's OpenSSL. Engine-team + 3p own the remaining merges. |
| 5: Bundling Library Exception filings | In prep. **Qt5 exception obsoleted for 26.10, but a Qt6 exception is now IN FORCE:** Qt6 merged into `development` 2026-06-15 ([#19567](https://github.com/o3de/o3de/pull/19567)) and the o3de-qt5 custom-rev9 bundle retires, but the intended system qt6-qtbase swap (Stage 1) is BLOCKED for shipping as of 2026-08-17 (bundled PySide6 pins the major-scoped `Qt_6_PRIVATE_API` symbol Fedora's Qt6 does not export), so 26.10 ships bundled Qt 6.10.2 and needs a Qt6 Bundling Library Exception until Option B (PySide6 rebuilt against system Qt6) lands. Remaining documented exceptions: squish-ccr (BC7 patent encumbrance), NvCloth (NVIDIA license); plus OpenSSL / bundled-Python / pyside pending their stages. libtiff dropped out 2026-06-05: Patch0016 (TIFF_DISABLE_DEPRECATED) dissolved the CryCommon int64 collision and system_tiff validated end-to-end on experimental (builds 10568964/10570032 + CI 27017382939). The DXC entry that previously sat in this list dropped out 2026-05-28: `o3de2605-dxc-spirv` (in `hellaenergy/o3de-dependencies`) is the license-clean rebuild, fits Fedora's accepted vendored-compiler precedent (emscripten, halide, hipcc), and replaces the binary bundle entirely. |

### Notable changes since this doc was last comprehensively updated

- **26.10 line live; `system_qt6` REVERTED to bundled Qt6 2026-08-17 (blocked on Option B).** `stabilization/26100` cut 2026-08-11 from development tip (9bc3b9d), 26.10 release targeting Oct 28. Direction (Nick 2026-08-12): system-swap posture across all COPR repos. The Stage-1 `system_qt6` swap built green on all 3 chroots, but the shipped RPM was UNINSTALLABLE on a clean Fedora box: the bundled PySide6 links the bundle's major-scoped `libQt6Core.so.6(Qt_6_PRIVATE_API)` and Fedora's Qt6 exports only the minor-scoped `Qt_6.11_PRIVATE_API` (o3de chair Roddie Kieley hit the uninstallable error; it had only resolved where a sibling bundled-Qt6 o3de package supplied the old symbol, a false pass). Dropping PySide6 to fix it would kill QtForPython + DccScriptingInterface, so 26.10 ships **bundled Qt 6.10.2** for now (self-consistent with bundled PySide6, keeps those features). Mechanics: `swap_hook` replaces `system_qt6` as the Patch0014 activator, so the 17-library Stage-1 swap set is UNCHANGED and still active; only Qt6 reverted. The `system_qt6` machinery stays in-tree default-off. Restore path = Option B (rebuild PySide6 against system Qt6, gated behind Stage-3 system Python); scoping in `upstream-drafts/`. **Cross-chroot availability probed 2026-08-12 vs the engine's qt-6.10.2 pin: NO blocker.** F44, F45, and rawhide (now F46) ship qt6 6.11.x (Fedora); CS10 ships the qt6 stack 6.10.1 in AppStream (base) + python3-pyside6/shiboken6 6.10.1 in EPEL10 (needs EPEL10 on the CS10 chroot). Spread 6.10.1..6.11.1, all one Qt6 major, all >= pin. Only risk: `-Werror` + Qt 6.11 deprecations on F44/F45/rawhide (standard `-Wno-error=deprecated-declarations`). Lua Patch0011 regenerated for 26100 (automoc context drift) 2026-08-12. The first 26100 stabilization build (COPR 10849347) failed %prep on it before reaching any Qt stage.
...
- `hellaenergy/o3de-development`: Sunday-cron builds from `o3de/development` (renamed from `o3de-snapshot` 2026-05-23 to make the branch-tracking intent explicit; auto-refreshed via `.github/workflows/snapshot-development.yml`). For an in-progress migration ref, a dedicated COPR project per branch (as was done for the Qt6 migration, which merged into development 2026-06-15; its `o3de-development-qt6` project was retired 2026-07-08).
...
**Status:** blocked on Stage 3 (Python migration to system Python) for the Python-C-module side. **BUT upstream is actively working OIIO now (Nick_L, 2026-07-23/25, #sig-build):** got OpenImageIO building as shared libraries that pack all their dependencies into their own .so (had to disable libuhdr, patch FindZLIB, add a build-dir wipe between configs). His stated Todo includes modifying O3DE to **drop openexr / libpng / libjpeg / libtiff references and route all image-format loading through OpenImageIO**, and drop the Qt image-format plugins from the asset pipeline. If that lands it is a big consolidation: several of our current Stage 1 swaps (openexr, png, and the Qt imageformat side) get subsumed by a single OIIO dependency. WIP signal, not merged; the Python-binding ABI dependency on Stage 3 likely persists for the editor's embedded interpreter. Also surfaced a general O3DE Find*.cmake scoping bug (targets created globally but find-vars set only in local scope, so a later `find_package()` in another scope returns early without the vars), relevant to our system-swap Find shims; captured in FOLLOW_UPS.
