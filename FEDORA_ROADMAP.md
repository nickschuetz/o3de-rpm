...
**Status:** **6-pack staged (2026-05-05).** Activated in `o3de-experimental`: `expat`, `freetype`, `lz4`, `mikkelsen`, `libpng`, `zlib`. The 5-pack baseline (validated 2026-05-04) gains `lz4`: Findlz4-system.cmake follows the mikkelsen pattern; engine consumers (Gems/MultiplayerCompression and Code/Framework/AzFramework Archive) use `#include <lz4.h>` / `<lz4hc.h>` / `<lz4frame.h>` verbatim, which match Fedora's lz4-devel layout exactly — no wrapper-header bridging needed. Engine binaries auto-Require `liblz4.so.1`. `libtiff` settled into Option C (Bundling Library Exception path) on 2026-05-05; `lua` remains deferred for an AzCore carry-patch.
