...
**Status:** **6-pack staged (2026-05-05).** Activated in `o3de-experimental`: `expat`, `freetype`, `mikkelsen`, `libpng`, `tiff`, `zlib`. `libtiff` joined the set with Patch0008 (CryCommon `int64`/`uint64` guard macro `O3DE_SYSTEM_LIBTIFF_COMPAT` plus `SKIP_UNITY_BUILD_INCLUSION` on the two TIFF .cpp files) and a FindTIFF-system mikkelsen-pattern refactor. The 5-pack baseline (validated 2026-05-04) is unchanged; system_tiff is the new addition this push. `lua` remains deferred for an AzCore carry-patch — write-up below.
  ...
  - **`libtiff` activated (2026-05-05)** — Patch0007 (deprecation migration of the engine's two `<tiffio.h>`-using files from libtiff's `uint8`/`uint16`/`uint32` typedefs to C99 names) was already in place. Patch0008 closes the second layer: libtiff 4.5+'s `<tiff.h>` defines `int64`/`uint64` as `int64_t`/`uint64_t` (`long` on LP64) while `Code/Legacy/CryCommon/BaseTypes.h` defines them as `slonglong`/`ulonglong` (`long long`). Same size, distinct C++ types → typedef redefinition error in any TU including both. Patch0008's narrow approach: a guard macro `O3DE_SYSTEM_LIBTIFF_COMPAT` around CryCommon's `int64`/`uint64` typedefs, defined-before-includes in the two TIFF `.cpp` files only, plus `SKIP_UNITY_BUILD_INCLUSION` on those two files so the macro doesn't leak into other unity merge group members. Avoids the engine-wide CryCommon C99 migration (Option B) that would otherwise be needed. FindTIFF-system.cmake also refactored to mikkelsen pattern (no stock include) for the same reason as the four ZLIB-class shims.
...
| libtiff | `libtiff-devel` | **validated 2026-05-05** (Patch0008 + FindTIFF-system refactor; Patch0007 stays in place for the deprecation layer; `O3DE_SYSTEM_LIBTIFF_COMPAT` guard macro narrows the CryCommon int64/uint64 conflict to two TUs via SKIP_UNITY_BUILD_INCLUSION) |
