...
**Status:** **6-pack validated end-to-end on `o3de-experimental` (2026-05-05).** Build 10426632 succeeded on F44 + rawhide; CI run 25402407670 passed Tiers 1+2+4+6 on both chroots. Activated swaps: `expat`, `freetype`, `lz4`, `mikkelsen`, `libpng`, `zlib`. The 5-pack baseline (validated 2026-05-04) gained `lz4`: Findlz4-system.cmake follows the mikkelsen pattern; engine consumers (Gems/MultiplayerCompression and Code/Framework/AzFramework Archive) use `#include <lz4.h>` / `<lz4hc.h>` / `<lz4frame.h>` verbatim, which match Fedora's lz4-devel layout exactly — no wrapper-header bridging needed. Engine binaries auto-Require `liblz4.so.1`. **Eligible for promotion to `o3de-stabilization`** whenever Mike's tester signal on the current stabilization build comes back positive (active-testers-window rule). `libtiff` settled into Option C (Bundling Library Exception path) on 2026-05-05; `lua` remains deferred pending AzCore investigation.
