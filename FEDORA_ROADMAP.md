...
**Status:** **mikkelsen-only baseline (2026-05-03, after multi-iteration discovery).** Activated in `o3de-experimental`: `mikkelsen` (validated independently in build 10419014). The other four candidates (`expat`, `freetype`, `libpng`, `zlib`) are deferred pending a Find-shim refactor; `libtiff` is deferred for a deeper CryCommon migration; `lua` is deferred for an AzCore carry-patch. Each has its own write-up below.

  - **`expat` / `freetype` / `libpng` / `zlib` deferred (2026-05-03)** — build 10421133 (5-pack activation) failed in cmake configure-time with `UNKNOWN_LIBRARY Library ZLIB::ZLIB specified MAP_IMPORTED_CONFIG_DEBUG = DEBUG; but did not have any of IMPORTED_LOCATION_xxxx set`. Each of these four `Find<X>-system.cmake` shims `include()`s cmake's stock find module (FindZLIB.cmake / FindPNG.cmake / FindFreetype.cmake / FindEXPAT.cmake) to delegate library/header lookup. The stock include creates a side-effect upper-namespace target (`ZLIB::ZLIB` etc.) as `UNKNOWN IMPORTED` with `MAP_IMPORTED_CONFIG_*` properties set but only the unconfigured `IMPORTED_LOCATION` populated. O3DE's runtime-dependency walker iterates every target in the project, hits those, and bails. **Mikkelsen alone works** because `Findmikkelsen-system.cmake` does its own `find_path` + `find_library` directly with no stock-module include — so `mikkelsen::mikkelsen` never exists, only `3rdParty::mikkelsen`. **Fix path:** refactor each of the four shims to follow the mikkelsen pattern (drop the stock include, do `find_path` + `find_library` inline). Bcond + find shim + Source declaration stay in place for future activation; the Patch0006 gating logic in `BuiltInPackages_linux_x86_64.cmake` is already correct for all four.
  - **`libtiff` deferred (2026-05-03)** — Patch0007 (which migrates the engine's two `<tiffio.h>`-using files from libtiff's deprecated `uint8`/`uint16`/`uint32` typedefs to C99) is in place and required regardless, but a deeper layer surfaced in build 10420962: libtiff 4.5+'s `<tiff.h>` defines `int64`/`uint64` as `int64_t`/`uint64_t` (`long` on LP64) while `Code/Legacy/CryCommon/BaseTypes.h` defines them as `slonglong`/`ulonglong` (`long long`). Same size, distinct C++ types → typedef redefinition error in any TU including both (Unity-build merging surfaces this widely). The fix needs a CryCommon migration to C99 typedefs — foundational header with audit cost across the engine (overload resolution, printf specifiers, template specializations). Parked as a separate Stage 1 item.
...
| zlib | `zlib-devel` | **deferred** — `FindZLIB-system.cmake` shim's stock-FindZLIB include creates side-effect `ZLIB::ZLIB` target that fails O3DE's runtime walker; refactor shim to mikkelsen pattern (direct `find_path`/`find_library`, no stock include) |
| freetype | `freetype-devel` | **deferred** — same shim-pattern issue as zlib (Findfreetype shim refactor) |
| libcurl | `libcurl-devel` | follow-on (use mikkelsen template) |
| libpng | `libpng-devel` | **deferred** — same shim-pattern issue as zlib (FindPNG shim refactor) |
...
| expat | `expat-devel` | **deferred** — same shim-pattern issue as zlib (Findexpat shim refactor); also retains its case-bridging role for `find_package(expat)` lowercase from bundled-FindOpenColorIO |
