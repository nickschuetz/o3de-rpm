...
The goal: get the stable `o3de` package into the **Fedora repository proper**. Until that's reached, COPR (`hellaenergy/o3de` + `hellaenergy/o3de-stabilization` + `hellaenergy/o3de-development`) is the interim distribution channel.
...
- `hellaenergy/o3de` — **tracks `o3de/o3de:main`** (upstream's release branch where each tagged release lives: `2510.2`, `2605.0`, etc.). Twice-yearly major cadence + occasional point releases. Post-release runbook lives at [`POST_RELEASE.md`](POST_RELEASE.md); `make release-stable` target pre-flights spec state before pushing.
- `hellaenergy/o3de-stabilization` — community-tester channel; pre-release validation builds from `stabilization/<release>`.
- `hellaenergy/o3de-development` — ad-hoc cadence builds from `o3de/development` (renamed from `o3de-snapshot` 2026-05-23 to make the branch-tracking intent explicit). For arbitrary other refs (e.g. qt6), create a dedicated COPR project per branch.
- `hellaenergy/o3de-experimental` — in-flight Stage 1 / Stage 2 migrations.

All four built with `enable_net=true` so O3DE's `LY_PACKAGE_SERVER_URLS` fetcher can pull the remaining restricted bundles (DXC + NvCloth + squish-ccr; poly2tri became Stage-1-swappable as of 2026-05-07) from `packages.o3de.org` (see "Restricted bundles" below).
