...
| **10** *(BLOCKED 2026-05-20)* | NewspaperDeliveryGame (Paper_Kid) build+bake smoke -- clones from `nickschuetz/NewspaperDeliveryGame` (fork) at a pinned SHA, registers the project against the installed engine, cmake configure, runs AssetProcessorBatch over the full project (2-pass absorber), launcher smoke under DISPLAY/Xvfb. Sister tier to Tier 9 but a different project shape: `script_only=true` (no native C++ gem code), single-player, heavy LyShine + LandscapeCanvas + WhiteBox + EMotionFX surface. Lower cost than Tier 9 (no native link phase). Uses Nick's fork rather than upstream directly so the `test` branch can carry Linux-specific fixes without diverging public-facing `main`. **NOT** part of `make test`; explicit-only via `make test-newspaper-delivery`. **Currently blocked on upstream LFS endpoint** ([o3de/NewspaperDeliveryGame#19](https://github.com/o3de/NewspaperDeliveryGame/issues/19) -- CloudFront returns 403 AccessDenied on every signed object URL; sig-build chair confirmed and is working on it). Script is staged for re-enable once LFS unblocks. | RPM installed, ~2-3 GB disk, network, clang/cmake/ninja | ~10-20 min cold / ~3-10 min warm |
