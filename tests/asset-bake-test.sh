...
run_ap_pass() {
    ...
    # Cold pass: serialize via --regset maxJobs=1 to sidestep the cold-cache
    # SRG-merge dependency-ordering race (viewsrg.srgi / scenesrg.srgi need
    # to be auto-generated by MergeShaderResourceGroupAsset BEFORE
    # ShaderAssetBuilder runs against them; with default parallelism the
    # builders race, causing 76+ FBX bakes + 210 shader builds to fail
    # spuriously on first AP pass). Warm pass runs at default parallelism
    # for speed.
    local regset_args=()
    if [ "$label" = "cold" ]; then
        regset_args+=( --regset "/Amazon/AssetProcessor/Settings/Jobs/maxJobs=1" )
    fi
