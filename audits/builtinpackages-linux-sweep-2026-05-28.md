...
| 2 | RapidXML-1.13-rev1-multiplatform | RapidXML | **swap-parked, blocked on patched-API divergence** | Stage 1 swap landed -82 then walked back -87 after experimental build 10522249 surfaced an API gap: the bundled `1.13-rev1` carries Amazon Lumberyard's "AZ Extention" `isError()` / `getError()` / `resetError()` methods on `xml_document` (the engine's `ObjectStream.cpp:2069-2071` directly uses `isError()` and `getError()`); Fedora's `rapidxml-devel` ships only upstream RapidXML which uses exception-based error reporting. O3DE builds with `-D_HAS_EXCEPTIONS=0` so the exception path is closed. Same shape as libtiff. Activation framework (bcond, Source46, Find shim, Patch0006 gate, BR/Recommends/-D blocks) stays in place defaulted off; reactivates when a license-clean COPR rebuild of the patched fork ships. |
...
- **swap-active**: 17 (the original 14-pack: expat, freetype, libpng, expat, lz4, mikkelsen, openexr, poly2tri, lua, assimp, sqlite, libsamplerate, googlebenchmark, vulkan-validation-layers; plus zlib; plus the multiplatform additions rapidjson in -83, xxhash in -84, cityhash in -85; rapidxml landed -82 then walked back -87)
- **swap-parked**: 2 (tiff, rapidxml; both blocked on patched-fork APIs that Fedora's upstream packages lack)
