...
| 0007 | `Gems/Atom/Asset/ImageProcessingAtom/.../TIFFLoader.cpp` | replace nine remaining legacy libtiff `uint32` typedef uses with standard C99 `uint32_t`. libtiff 4.5+ marks the legacy typedef `__attribute__((deprecated))`; with O3DE's `-Werror` every stale use is a hard build failure. Mechanical type rename; same underlying type. Adjacent code already uses `uint32_t` (partial migration upstream) — this finishes it. | **yes** — required for any build against modern libtiff (Fedora 44+, Ubuntu 24.04+, etc.); conservative; finishes a migration upstream already started |
