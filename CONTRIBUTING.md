...
- Patch0004 (LYPython sdist install for read-only installed engine) <- [#19752](https://github.com/o3de/o3de/pull/19752) merged 2026-06-09
...
The five engine projects all set `enable_net=true` (so cmake can fetch the remaining bundles from `packages.o3de.org` at build time; current set: NvCloth + squish-ccr restricted-by-license, plus OpenSSL + OIIO/OCIO + the unaudited multiplatform deps still in transit toward Stage 1 or Stage 2 coverage; on the 26.10 line also **bundled Qt6 6.10.2 + PySide6/shiboken6** since the system-Qt6 swap is reverted pending Option B, and on the 26.05 line the custom Qt 5.15 + pyside2) and pull `o3de-dependencies` via the chroot's `additional_repos` (build-time) and `runtime_dependencies` (consume-time, for end users).
...
Each engine project hosts the **versioned** package(s) that match whatever the spec's `stable_tag` resolves to at build time. `stable_tag` is now `2610.0`, so `o3de-stabilization` + `o3de-development` produce **`o3de2610`** (26.10 line, `/opt/O3DE/26.10.0/`); the `o3de` stable + `o3de-testing` channels still ship `o3de2605` (26.05.0, the tagged release). Both majors co-install. The COPR project name (`o3de`, `o3de-stabilization`, …) is the *channel*; the package name (`o3de2605`, `o3de2610`, …) is the *major*. They're orthogonal.
...
- **Target distros:** Fedora 44+ and CentOS Stream 10+ only. No F43/CS9-or-earlier shims. (Earlier project framing said "RHEL 10+"; that was shorthand for the CS10 line, which is upstream of RHEL 10.) Built today: `fedora-44`, `fedora-45`, `fedora-rawhide` (= Fedora 46, since F45 branched off rawhide 2026-08-17), `centos-stream-10`. COPR auto-adds a `fedora-<N>` chroot when Fedora branches; reconcile its `--rpmbuild-with` set on every project after a branch.
- **Qt is bundled on both lines, differently.** 26.05 bundles a custom-patched Qt 5.15-rev9 (rev9 patches load-bearing; never add system Qt5 BRs/Requires). 26.10 bundles *vanilla* Qt 6.10.2: the `system_qt6` swap is implemented but reverted (default-off) because the bundled PySide6 pins the upstream-major `Qt_6_PRIVATE_API` symbol Fedora's Qt6 does not export; system Qt6 returns with Option B (rebuild PySide6). See `BUNDLED_LIBRARIES.md` + `project_o3de_bundles_custom_qt.md`.
