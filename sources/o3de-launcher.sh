...
# Manifest migration (every launch, no marker gate — idempotent and only
# does work when there's actually stale state). Repoints any `engines_path`
# entry in ~/.o3de/o3de_manifest.json that maps to a known legacy prefix
# (e.g. "o3de" -> "/opt/o3de") to the current versioned install path.
# Without this, pre-versioned-rename projects with `"engine": "o3de"` in
# their VCS-tracked project.json fail to resolve an engine because the
# manifest still claims o3de -> /opt/o3de — a path that doesn't exist
# after the rename's pre-rename `o3de` package is removed. Repointing
# the manifest keeps the user's project.json untouched (no VCS noise)
# while letting Project Manager's "Open Editor" path resolve the
# legacy engine name to the current engine root.
#
# Why no marker gate: the rewrite is a no-op when the manifest has no
# stale entries, so re-running on every launch costs ~10ms python
# startup and zero filesystem writes. Cheaper than a marker that goes
# stale across legacy-prefix sets or migration-logic revisions.
...
try:
    ...
    sys.exit(0)
ep = m.get("engines_path")
if not isinstance(ep, dict):
    sys.exit(0)
changed = False
for name, path in list(ep.items()):
    if path in legacy_prefixes and path != target:
        ep[name] = target
        changed = True
if changed:
    try:
        ...
        pass
' "$ENGINE_PATH" "$manifest" 2>/dev/null || :
fi
