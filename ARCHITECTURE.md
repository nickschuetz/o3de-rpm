...
flowchart TB
    ...
    subgraph DIST["Distribution channels"]
        ...
        DC1S["COPR<br/>hellaenergy/o3de-development<br/>(engine o3de/o3de:development;<br/>Sunday cron, pinned to the<br/>o3de-rpm development spec branch)"]
...
5. **One spec, two branches, many channels.** The same `o3de.spec` lives on two branches: `main` (stable/release packaging, the repo default) and `development` (dev-channel packaging: dev-only carry-patch gating like the `Patch0004`/o3de#19752 gate, plus the qt6-merge tooling and the default-off `wayland` bcond). The stable-facing channels (`o3de` stable, `o3de-testing` pre-promotion soak, `o3de-stabilization` community-tester, `o3de-experimental` in-flight migration) build from `main`; the development channels (`o3de-development`, `o3de-development-debug`, and the transitional `o3de-development-qt6`) build from `development`. Only `o3de-development` is automated, via the Sunday `snapshot-development.yml` cron, which pins its checkout to `development`; the rest are manual `make` targets run from the matching branch. The spec also feeds the eventual o3debinaries.org upstream submission and Fedora itself; a future Flatpak reuses most of the tree with its own manifest. A channel marker in the GUI version string (`-stabilization`, `-development.<commit>`, `-experimental.<commit>`, or empty for stable and testing) tells a tester which channel a build came from at a glance.
