...
- **Stage 1/2a 7-pack** — engine links to system `expat`, `freetype`, `liblz4`, `libpng`, `mikkelsen` (`libmikktspace.so.0`), `openexr` (+ `imath`), `zlib` instead of bundled copies. The OpenEXR + Imath addition is the first cross-stage step (Stage 1 → Stage 2a). Validated end-to-end on 2026-05-07: build 10430726 succeeded on F44 + rawhide, CI passed Tiers 1+2+4+6 on both chroots. Find-shim pattern (direct find_path/find_library, no stock-cmake-include, real INTERFACE IMPORTED 3rdParty target — keeps O3DE's runtime-dependency walker happy) is uniform across all 7 swaps; OpenEXR adds a two-shim variant (`FindOpenEXR.cmake` + `FindImath.cmake`) because the bundled FindOpenColorIO calls `find_package(Imath)` independently of OpenEXR.
...
- `system_lua` — deferred; AzCore's `ScriptContext.cpp` includes Lua's *internal* `<Lua/lobject.h>` header, which Fedora's `lua-devel` doesn't ship (only public API). Resolution path TBD — could be an AzCore public-API migration upstream, or a carry-patch if the internal types are load-bearing. Investigation in flight.
