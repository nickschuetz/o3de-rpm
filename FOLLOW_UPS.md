...
**MultiplayerSample post-load crash (caught by Tier 11 on 2026-05-22, filed upstream as [o3de/o3de#19780](https://github.com/o3de/o3de/issues/19780))**: After `LEVEL_LOAD_END` of `Levels/startmenu/startmenu.spawnable`, the launcher aborts within seconds with SIGABRT. Actual crash frame is `UiCanvasAssetRefComponent::LoadCanvas` in libLyShine.so (not the `SetEffectiveActiveLayerByTypeIndex` warning that fires earlier; that one is a separate bootstrap-order issue from PR #19319 and returns false harmlessly). Repro: `make test-tier11-multiplayer`. Workaround for testing: `LEVEL=gameplaytest make test-tier11-multiplayer` (passes Tier 11 cleanly, runs 30+ s).

JT_SCB_GameDesign (Discord, 2026-05-22) confirmed this is a KNOWN issue community devs have privately fixed but never upstreamed. Windows users see it intermittently (per JT: "you have to just start the game from the bail out window UI 2-3 times then it will be have... Once it the game, I played it for 2.5 hrs straight w/o fail"). Linux is deterministic, which gives us a cleaner repro than the Windows community has had. JT also noted the downloadable MPS Dg is a 23.x Windows build, which explains how the 25.x/26.x experience drifted. JT will comment on the upstream issue for additional eyes.

Tier 9 was originally claiming "passes end-to-end" because its smoke check only verified the level-load success marker was reached at some point; doesn't verify post-load liveness. Tier 11 added 2026-05-22 catches this gap. Yesterday's commits walked back the "MPS works" framing across README + CONTRIBUTING + tests/README + ARCHITECTURE.
