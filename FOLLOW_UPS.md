...
UPDATE 2026-08-04: **#19947 finding ACCEPTED + FIXED.** Nick_L replied "Nicely spotted" (comment 5185790222) and pushed 9a4b7565 ("Updated based on PR comments", DocumentPropertyEditor.cpp only). Verified the fix: label branch now `delete label` (was the null `delete rowWidget`), restructured into symmetrical typed branches (row / label), PLUS a new catch-all `else { delete childWidget; }` for unknown widgets so nothing leaks. Complete + correct, our finding resolved. **#19954 NOT yet revised** (still at 989ec41); watcher relaunched (baselines: #19947 head 9a4b7565, #19954 head 989ec41) to catch the ScriptOnly fix so we can re-run the create-project test to confirm the crash is gone + script_only restored. Kyler flagged (sig-build Discord 2026-08-04) that Qt6 causes GraphCanvas/Script Canvas crashes on node drag/remove/connect -- same re-entrancy CLASS as #19947 but a different widget stack (GraphCanvas, not DPE); potential Qt6-in-26.10 concern for the migration we drove; offered to investigate, awaiting Nick's go.
