...
**MPS gameplay crash: AudioSystemAllocator exhaustion under sustained play (caught 2026-05-22 14:14 CDT)**: After verifying multiplayer works end-to-end (server + client, NewStarbase, ROUND 1 of 3, player character active), continued play for several minutes resulted in both client and server crashing with SIGABRT within ~9 seconds of each other. Game.log shows hundreds of `[Error] (System) - Failed to get a new instance of an AudioObject from the implementation. If this limit was reached from legitimate content creation and not a scripting error, try increasing the Capacity of Audio::AudioSystemAllocator.` entries in the seconds leading up to the crash. The error message itself documents the cause: default `AudioSystemAllocator` capacity is sized for short sessions; sustained gameplay (gem pickup chimes, laser fire, jump-pad whooshes etc.) exhausts the pool. Secondary symptoms: `Autonomous Desync` (netcode reconciliation, normal under load) and `Processing time exceeded, discarding 171/382 received packets` (server fell behind because audio errors slowed the main loop -- cascading consequence, not root cause). NOT a Linux-specific bug, NOT related to our packaging, NOT the gem-double-load PR #501/#502 issue. Probably hits Windows users too under similar sustained play. Engine-side: tunable via `/Audio/Capacity` settings registry or similar. Crash artifacts preserved at `/tmp/mps-crash-2026-05-22/` (Game.log, client-stdout, server-stdout) for future analysis. Not filing upstream right now; the engine's own error message is self-documenting and JT-and-friends likely know about this class of issue. Parked here for the record + as a reminder if the topic comes up.
