...
UPDATE 2026-08-04: **BOTH findings ACCEPTED + FIXED + VERIFIED. Review loop CLOSED.**
- **#19947:** Nick_L "Nicely spotted" (comment 5185790222), pushed 9a4b7565 (DocumentPropertyEditor.cpp). Fix verified in diff: label branch now `delete label` (was null `delete rowWidget`), symmetrical row/label typed branches, PLUS a catch-all `else { delete childWidget; }` so nothing leaks. Correct + complete.
- **#19954:** Nick_L "Nicely spotted! I've uploaded fixes for all above" (comment 5185867201), pushed 4f885df2. Addresses ALL THREE of our points incl. the packaging heads-up: (a) removed the EngineFinder copyFiles entry from ScriptOnly template.json (crash), (b) added the script_only -> O3DE_SCRIPT_ONLY block to the shared EngineFinder template (property loss), (c) added an install rule + EngineFinder subdir to Templates/CMakeLists.txt autoinstall (our SDK-ship/register heads-up). RE-RAN create-project against 4f885df2 in the isolated venv/manifest (real ~/.o3de untouched): ScriptOnly create-project now SUCCEEDS (was FileNotFoundError) and the generated cmake/EngineFinder.cmake sets O3DE_SCRIPT_ONLY. Both confirmed end-to-end.
- Watcher stood down (both PRs revised + verified; review complete). Relaunch only if we want merge/CI tracking.
- STILL OPEN (offered, awaiting Nick's go): Kyler flagged (sig-build Discord 2026-08-04) Qt6 causes GraphCanvas/Script Canvas crashes on node drag/remove/connect, same re-entrancy CLASS as #19947 but a different widget stack (GraphCanvas, not DPE); potential Qt6-in-26.10 concern for the migration we drove.

**#19954 Template Inheritance (https://github.com/o3de/o3de/pull/19954): TWO confirmed findings, ran create-project on the PR branch (989ec41) in an isolated venv + isolated o3de manifest (real ~/.o3de untouched):**
1. SHOWSTOPPER (runtime-confirmed): create-project from ScriptOnlyProject fails with FileNotFoundError. The PR deletes Templates/ScriptOnlyProject/Template/cmake/EngineFinder.cmake but ScriptOnlyProject/template.json still lists it in copyFiles (line 39) + the cmake dir in createDirectories (line 302). Those entries were correctly removed from DefaultProject + MinimalProject but missed on ScriptOnly. DefaultProject create-project succeeds (control). Fix: drop those two entries.
2. Under that (also confirmed): the shared EngineFinder template (= DefaultProject's copy) lacks the script_only detection block that ScriptOnly's own copy had; on current development the ScriptOnly EngineFinder line the PR deletes is the ONLY repo-wide setter of the O3DE_SCRIPT_ONLY global property (verified via GitHub code search + raw fetch; consumers include CompilerSettings.cmake, root CMakeLists.txt, 3rdPartyPackages, PAL_linux, LYWrappers_linux). So a fixed-#1 ScriptOnly project would still lose script-only mode and build as a full native project. Fix: fold the script_only block into the shared EngineFinder template. This matters to us: we ship the templates in the RPM and steer no-toolchain Lua/ScriptCanvas users to ScriptOnly.

**#19947 Prefab crash/flicker fix (https://github.com/o3de/o3de/pull/19947):** core use-after-free fix is sound (drop cached component ptr on entity deactivate, re-fetch by id on activate; queued reset instead of synchronous NotifyResetDocument that deletes `this`). GUT-CHECK caught + KILLED my original headline concern: I first thought the new `if(!m_isResetQueued) return;` guard in NotifyResetDocument would break the meta-adapters (Filter/Sort/Aggregate) that call NotifyResetDocument directly without queuing. REFUTED by full trace against current development: MetaAdapter connects a ConnectResetQueuedHandler that calls NotifyResetQueued, and the PR MOVES `m_isResetQueued = true` into NotifyResetQueued, so the flag propagates up the whole meta chain synchronously and the guard passes by the time HandleReset->NotifyResetDocument runs. (Lesson: my local ~/PROJECTS/o3de was ~2 months stale on `tiff-disable-deprecated`, predating the queue mechanism; re-verify DPE reset claims against current development, not local.) Only surviving note: in the widget-pooling-off branch (new default) the label case does `delete rowWidget` (a null failed-cast) instead of `delete childWidget` like the sibling block, likely masked by Qt parent ownership. Decided (Nick) to SKIP the hours-long PR-branch engine build for a crash-repro demo since the reset concern was settled by trace.
...
BLOCKER CONFIRMED 2026-08-03: the experiment is gated UPSTREAM: there is no 3.6.3 package to pin against yet. Evidence: (1) no o3de/o3de PR bumps the OpenSSL association to 3.x (newest is #19520, Jan 2026, still 1.1.1w for mac-arm64), so the engine source has NOT been re-pointed at 3.x and there is no CDN hash to borrow; (2) probed the O3DE 3rdParty CDN directly (base `https://d3t6xeg4fgfoum.cloudfront.net`, from cmake/3rdPartyPackages.cmake:32): `OpenSSL-1.1.1t-rev1-linux.tar.xz` -> HTTP 200 (format confirmed), `OpenSSL-3.6.3-rev{1,2,3}-linux.tar.xz` -> HTTP 403 = not present (this bucket returns 403 AccessDenied for a missing key because ListBucket is denied, so 403 vs the known-good 200 is the not-present signal). #387 merged the 3p BUILD RECIPE but the built Linux binary package has not been published to the CDN, and no association PR exists. So to actually RUN the probe, either (a) WAIT for O3DE to publish the 3.6.3 package + open the o3de/o3de association PR (natural #387 follow-through), or (b) BUILD the OpenSSL-3.6.3 3rdParty package ourselves from the merged recipe, host it, point a BuiltInPackages carry-patch at it, confirm the engine source builds against 3.x (EncryptionCommon/AzNetworking; #9441 did 3.0-compat prep in 2022, unverified against 3.6), THEN experimental = development + monolithic + that patch. Option (b) is a real multi-step spike that front-runs upstream. Experimental's chroots UNTOUCHED pending Nick's a-vs-b call; it currently mirrors development (harmless).
...
## UPDATE (2026-08-05 later): #19977 is a partial dup of #19966; PR #19978 EXPANDED to fix all 3

From the sig-release Discord: Matteo flagged existing script-canvas issues #19972 (unrelated, Save/Save-As bug) and #19966 ("NodePallete QT regression bugs", jabbadablack, 2026-08-02, Windows 11). #19966 is the PRIOR report of the SAME add-node palette crash we filed as #19977 PLUS two more NodePalette bugs. My pre-filing prior-art search missed it (misspelled title "NodePallete"). #19966 also confirms the crash on Windows -> cross-platform, not Linux-only.

ACTION (Nick chose expand+crosslink): expanded PR #19978 to fix ALL THREE #19966 bugs, re-validated on the local Qt6 editor build (add nodes; type "Examples"/"Get" in the palette search then CLEAR it = the SetFilter("") OOB path; delete/connect/drag), 0 faults. The three fixes: (1) crash = beginFilterChange/endFilterChange bracket in SetFilter/ClearFilter + drop external invalidate() [our approach; #19966 proposed clear-view-state-before-invalidate instead]; (2) filterAcceptsRow null-internalPointer guard; (3) SetFilter m_filter[0] empty-string guard. Amended to one DCO-signed commit 1502ab71e9, force-pushed; PR title/body updated to "Addresses #19966 + #19977". Patch now 2 files, +27/-8.
PENDING Nick's go: cross-link comments drafted in upstream-drafts/graphcanvas-19966-crosslink-comments.md: (a) acknowledge #19966 + @jabbadablack on that issue, (b) close #19977 as dup of #19966. NOT posted.
...
I closed #19977 as a dup of #19966 with reason NOT_PLANNED. Nick flagged that "Closed as not planned" reads as dismissive for a real, actively-fixed crash (bad signal to reporter/triage). Correct: reopened #19977 (state OPEN/REOPENED) with a comment noting it's being fixed in #19978 (consolidated with #19966). Plan: leave #19966 + #19977 both OPEN + cross-linked; close them as COMPLETED when #19978 merges (or flip the PR to "Fixes #19966" at merge for auto-close). Lesson: for a genuine bug being fixed, do NOT close its issue as not-planned just because it is a duplicate; leave it open (or close as completed once the fix lands) so the badge does not read as "won't fix".
