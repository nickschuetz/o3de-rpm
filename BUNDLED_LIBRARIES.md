...
**Promotion to o3de-snapshot:** once an `o3de-experimental` build with `--with system_mikkelsen` passes the test-installed.yml workflow (Tier 1+2+3+6 across F44 + rawhide), the same SRPM can be uploaded to `o3de-snapshot` for community-tester validation — but only when Nick signals the testing window is open for new pushes. See `MEMORY.md` → "Active community testers on COPR".
