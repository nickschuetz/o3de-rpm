...
**Status as of 2026-05-08**: SPIRV-Cross PoC ✓ green (since 2026-05-07); **engine-side glue activated 2026-05-08** via `system_spirvcross` bcond (Stage 2 first-of-its-kind binary-only swap; `%install` symlinks the engine's runtime path to `/usr/bin/spirv-cross`). DXC PoC ✓ green (since 2026-05-08, rev12 / build 10435628), full end-to-end functional verification: `dxc -spirv -T ps_6_0 -E main shader.hlsl` produces valid SPIR-V output linking to system spirv-tools/spirv-headers + bundled DirectX-Headers. Engine-side glue for DXC follows the same install-overlay shape, pending a follow-up commit. mcpp added to this section 2026-05-08 after audit reframing: same DXC-class category but library-link shape (not binary shellout); PoC deferred, pattern proven, just bandwidth-bound.
