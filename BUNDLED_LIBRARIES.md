...
| **libtiff** | 4.2.0.15-rev3 | `libtiff-devel` | 4.6+ | **DEFERRED.** Two-layer issue: (1) Patch0007 already migrates `<tiffio.h>`'s deprecated `uint8`/`uint16`/`uint32` typedefs to C99 in `TIFFLoader.cpp` + `Code/Editor/Util/ImageTIF.cpp` (required regardless of system_tiff); (2) libtiff 4.5+'s `<tiff.h>` defines `int64`/`uint64` as `int64_t`/`uint64_t` (`long` on LP64) but CryCommon's `BaseTypes.h` defines them as `slonglong`/`ulonglong` (`long long`) — same size, distinct C++ types → typedef redefinition error. Fix needs CryCommon→C99 typedef migration. Plan filed at `linear-finding-shimwise.md` sibling `squeezing-typeface-tiffany.md`. Bcond + Source declaration stay in place. |
