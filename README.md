...
- **Snapshot builds**, from any git ref of `o3de/o3de`. Two upstream branches are common targets and they're *not* the same thing:
  - **`stabilization/<X>`** (currently `stabilization/26100`, the 26.10 line cut 2026-08-11), the pre-release stabilization branch for the next tagged release. Ships to **`hellaenergy/o3de-stabilization`** during the upstream stabilization window (typically a 4-week period before each release tag). Dormant between cycles. When O3DE upstream tags the release (e.g. `2610.0`), this branch's tip *is* the release, so snapshots from here are functionally release candidates.
  - **`development`**, the bleeding-edge integration branch where new features land daily. Ships to **`hellaenergy/o3de-development`**. Its chroots carry `development_snapshot qt6 monolithic`, so the build includes the monolithic (release, static) engine permutation and a release game export works from the installed RPM (the `monolithic` bcond graduated here from `o3de-experimental` on 2026-08-03 after two green builds across a dev-tip movement + an end-to-end export validation). Less stable than a stabilization branch; useful for engine contributors testing in-progress work, less appropriate for community testers expecting near-release quality.
...
#    TAG must match `%global stable_tag` in o3de.spec. stable_tag is currently
#    2610.0 (the 26.10 line, in stabilization; the 26.10.0 tag lands at release,
#    targeting Oct 2026). The example below builds the released 26.05.0, so set
#    stable_tag=2605.0 to match it.
TAG=2605.0
...
Both paths use `--with snapshot` under the hood; the project split is a publishing-channel choice, not a build-mode choice. See the bullets at the top of this README for the upstream-branch distinction. o3de-rpm itself is single-branch (`main`); the channel a build lands in is chosen by its bconds and the upstream engine ref it targets, not by a packaging-repo branch.

```bash
# 1. Generate a reproducible snapshot tarball + checksum.
cd sources
./make-snapshot-tarball.sh stabilization/26100   # next-release branch → o3de-stabilization
...
The resulting version string is `<stable_tag>^<YYYYMMDD>git<shortsha>` (e.g. `2610.0^20260814gita189c8a`). The `^` separator tells `dnf` this is a *pre-release* of the next release, so upgrading from a `--with snapshot` build to the next tagged release works correctly.
...
- **[`hellaenergy/o3de-dependencies`](https://copr.fedorainfracloud.org/coprs/hellaenergy/o3de-dependencies/)**. Fedora-clean SRPMs for O3DE 3rdParty packages that aren't in Fedora proper (custom Qt 5.15-rev9, PhysX, AWSNativeSDK, azslc, mikkelsen, the Stage 2 rebuilds o3de-dxc-spirv / o3de-spirv-cross / o3de-mcpp-az / o3de-cityhash, ...). `enable_net=false`. Built first; consumed by the engine projects via `additional_repos` at build time and `runtime_dependencies` at consume time (so users get it auto-enabled when they enable any engine project).
...
- **[`hellaenergy/o3de-stabilization`](https://copr.fedorainfracloud.org/coprs/hellaenergy/o3de-stabilization/)**. pre-release validation builds from upstream's `stabilization/<release>` branch. Active during the 4-week pre-release window when an upstream release is being stabilized (`stabilization/26050` was active April-May 2026; **currently active for `stabilization/26100`, the 26.10 line cut 2026-08-11, targeting an Oct 28 release**). Dormant between release cycles. The community testers' channel during active windows.
- **[`hellaenergy/o3de-development`](https://copr.fedorainfracloud.org/coprs/hellaenergy/o3de-development/)**. ad-hoc cadence builds from upstream's `development` branch (bleeding-edge engine). Chroots carry `development_snapshot qt6 monolithic` (the `monolithic` bcond was promoted here from `o3de-experimental` on 2026-08-03), so the build ships the monolithic (release, static) engine permutation and a release game export works from the installed RPM. For arbitrary other engine refs (e.g. a one-off feature branch or release candidate under test) we'd create a dedicated COPR project per branch rather than overload this one.
- **[`hellaenergy/o3de-experimental`](https://copr.fedorainfracloud.org/coprs/hellaenergy/o3de-experimental/)**. packagers' sandbox for work not yet ready for the community channels. Realigned 2026-07-24 onto `o3de/development` + the `monolithic` bcond (its chroots carry `development_snapshot qt6 monolithic`), off its former Stage-1 system-swap base (`stabilization/26050` shipped as 26.05.0 and is frozen; the o3de-stabilization channel now tracks `stabilization/26100`, cut from development tip 2026-08-11). It now builds the same dev-snapshot as `o3de-development` plus the monolithic (release, static) engine permutation, so a release game export works there. Distinguished by the `-development.<commit>` channel marker. Not for end-user testing; internal to the packaging effort.
...
All three engine projects use `enable_net=true` so cmake can still fetch the remaining bundles from `packages.o3de.org`. The licensing-restricted set is now two packages (NvCloth's NVIDIA license, squish-ccr's BC7 patent encumbrance); DXC retired from this set on 2026-05-08 when the license-clean DXC rebuild shipped in `o3de-dependencies`. `poly2tri` was a fourth restricted entry until the 2026-05-07 audit reframed it as a Stage 1 swap candidate; it now resolves through Fedora's `poly2tri-devel` when the swap is active.
...
See [`FEDORA_ROADMAP.md`](FEDORA_ROADMAP.md) for the staged plan. Six stages from system-lib migration through OpenSSL 3.x port, debuginfo subpackages, and Bundling Library Exception filings (current set: Qt 5.15-rev9 (26.05 line), Qt6 6.10.2 (26.10 line: the system_qt6 swap is implemented but blocked for shipping by the bundled PySide6's `Qt_6_PRIVATE_API` pin, so Qt6 ships bundled until Option B), squish-ccr, NvCloth; DXC retired 2026-05-28 after the license-clean rebuild shipped; libtiff retired 2026-06-05 when Patch0016's TIFF_DISABLE_DEPRECATED unblocked the system_tiff swap) before the package review submission.
...
```bash
make snapshot REF=stabilization/26100    # generate tarball + print pin values
...
make copr-development                    # SRPM → hellaenergy/o3de-development (dev-branch tip + monolithic)
make copr-development-and-test           # same + watch + fire CI tests
make copr-experimental                   # SRPM → hellaenergy/o3de-experimental (development + monolithic)
...
cd o3de-rpm
make test-branch REF=stabilization/26100   # or any git ref / release tag
```
...
| 0007 | `.../TIFFLoader.cpp` + `Code/Editor/Util/ImageTIF.cpp` | Migrate legacy libtiff `uint8`/`uint16`/`uint32` typedefs to standard C99 (libtiff 4.5+ deprecates the legacy names). **TIMEBOMB:** upstream merged [#19734](https://github.com/o3de/o3de/pull/19734) into `development` on 2026-05-08 but not into `stabilization/26050`; retires when the channel rebases onto the 26.10 base (see the retirement note above the table). |
| 0008 | `Code/Framework/AzCore/.../ScriptContext.cpp` | Drop the redundant `<lua/lobject.h>` include broken by Lua 5.5 layout changes. **TIMEBOMB:** upstream merged [#19733](https://github.com/o3de/o3de/pull/19733) into `development` on 2026-05-08 but not into `stabilization/26050`; retires when the channel rebases onto the 26.10 base (see the retirement note above the table). |
...
| 0017 | `Gems/LmbrCentral/.../lrelease_linux.cmake` | Skip the LmbrCentral lrelease POST_BUILD rpath-patch under `LY_USE_SYSTEM_QT` (system lrelease has no rpath; the `$ORIGIN/../lib` `RPATH_CHANGE` fatally fails). `system_qt6`-only, so **currently inactive** since the 2026-08-17 revert to bundled Qt6. Upstream-worthy (system-Qt build path). |
...
- Two upstream-bundled packages (`NvCloth`, `squish-ccr`) cannot be hosted in Fedora or COPR for licensing reasons (NVIDIA license; BC7 patent encumbrance). The Fedora-shippable variant routes around them via feature-gated builds; see [`FEDORA_ROADMAP.md`](FEDORA_ROADMAP.md) § "Restricted bundles". `DirectXShaderCompilerDxc` was a third entry until 2026-05-08, when the license-clean DXC rebuild shipped in `hellaenergy/o3de-dependencies` and replaced the bundle via the `system_dxc` swap. `poly2tri` was a fourth entry until the 2026-05-07 audit reframed it as a Stage 1 swap; it now resolves via Fedora's `poly2tri-devel`.
