...
The interim distribution channel. Five engine COPR projects plus a dependencies project, each with a distinct purpose:
...
See [`FEDORA_ROADMAP.md`](FEDORA_ROADMAP.md) for the staged plan. Six stages from system-lib migration through OpenSSL 3.x port, debuginfo subpackages, and Bundling Library Exception filings (current set: Qt 5.15-rev9, libtiff, squish-ccr, NvCloth; DXC retired 2026-05-28 after the license-clean rebuild shipped) before the package review submission.
...
- **Qt 5.15-rev9 auto-DPI mis-detection on Wayland XWayland.** Without `QT_FONT_DPI=100`, fonts in Project Manager render at the wrong scale on some HiDPI / 4K Wayland setups. The launcher now sets this automatically on Wayland sessions (when `XDG_SESSION_TYPE=wayland` and the user hasn't already set `QT_FONT_DPI`). The upstream Debian .deb package needs the same workaround. Manual override is still possible via `export QT_FONT_DPI=<value>` before launch.
- **Editor GUI helper dialogs need the Tk 8.6 runtime on Fedora 44.** Open Export Settings, Open Android Project Generator, and Open CMake GUI drive the editor's bundled Python via `tkinter`, whose `_tkinter` is built against Tk 8.6 (`libtk8.6.so` / `libtcl8.6.so`). Fedora 44 moved the default `tk` / `tcl` to 9.x and split the 8.6 runtime into the `tk8` / `tcl8` packages, and Export Settings additionally needs the `tix` package. A default `dnf install o3de2605` pulls `tk8`, `tcl8`, and `tix` in automatically via `Recommends:`, and the launcher sets `TCLLIBPATH=/usr/lib64/tcl` so the bundled Tcl finds Fedora's Tix (which installs outside the bundled Tcl's default search path). Minimal installs that opt out of weak deps (`--setopt=install_weak_deps=False`) must install `tk8 tcl8 tix` by hand for those dialogs. Community report on Fedora 44, 2026-05-29.
- **Editor windows do not respond to GNOME / KDE mouse-drag tile-snap gestures on Linux.** Dragging the Editor's title bar to a screen edge releases at the cursor instead of half-tiling; dragging to the top releases instead of maximizing. Project Manager (same Qt 5.15 bundle) interacts with the window manager normally; only the Editor is affected. Root cause is `AzQtComponents::WindowDecorationWrapper` overriding `nativeEvent()` to manage cross-platform chrome, which swallows the WM's tile-snap drag gesture before Qt forwards it. Workaround: keyboard shortcuts go through a different code path and work correctly. On GNOME: `Super+Up` to maximize / un-maximize, `Super+Left` / `Super+Right` to half-tile, `Alt+F7` to enter keyboard-driven move mode. Covers most of the tile-snap UX. KDE bindings are similar (`Meta+Up`, `Meta+Left`, `Meta+Right` by default).
