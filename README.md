...
- **[`hellaenergy/o3de-experimental`](https://copr.fedorainfracloud.org/coprs/hellaenergy/o3de-experimental/)** — packagers' staging channel. In-flight spec changes (future Stage 3+ migrations like OpenSSL 3, new `system_<X>` swap candidates, structural spec rework) validate here before promotion to stabilization. Currently builds the same `stabilization/26050` source as the stab channel with the same Stage 1 + Stage 2 swap set, distinguished only by the `-experimental.<commit>` channel marker (see [`FEDORA_ROADMAP.md`](FEDORA_ROADMAP.md)). Not for end-user testing; internal to the packaging effort.
